...
import { isFromAdmin } from "../../lib/telegram.js";
...
export async function handleCommand(
  ...
  switch (command) {
    ...
    case "/tasks":
      await import("./callbackHandler.js").then((m) =>
        m.handleShowTasks(user, chatId, 0)
      );
      break;

    case "/reminders":
      await import("./callbackHandler.js").then((m) =>
        m.handleShowReminders(user, chatId)
      );
      break;

    case "/history":
      await import("./callbackHandler.js").then((m) =>
        m.handleShowHistory(user, chatId, 0)
      );
      break;

    case "/stats": {
      const stats = await getStats(user.telegram_user_id);
      await sendMessage(chatId, statsMessage(stats), mainMenu());
      break;
    }
    ...
    case "/admin":
      ...
      await sendMessage(
        chatId,
        adminDashboardMessage(adminStats),
        adminPanel()
      );
      break;
