...
export function formatRelativeTime(
  ...
  if (diffHours < 24) {
    ...
    return remainMinutes > 0
      ? `${diffHours}h ${remainMinutes}m ago`
      : `${diffHours}h ago`;
  }
...
export function formatTimeRemaining(
  ...
  if (diffHours < 24) {
    return remainMinutes > 0
      ? `${diffHours}h ${remainMinutes}m remaining`
...
export function parseCustomTime(
  ...
  if (dateMatch) {
    ...
    if (!isValid(result)) {
      return { date: now, error: "Invalid date/time." };
    }
    return { date: result };
  }
...
export function parseCustomReminder(
  input: string
  ...
  for (const [regex, multiplier] of patterns) {
    ...
    if (match) {
      ...
      if (value > 0 && value <= 10080) {
        return { minutes: value * multiplier };
      }
    }
  }
  ...
  return {
    minutes: 0,
    error: `I couldn't understand that duration.\n\nPlease use formats like:\n<code>30 minutes</code>\n<code>2 hours</code>\n<code>1 day</code>`,
  };
}

export function isTimeInPast(date: Date): boolean {
